...
11. [CS-046] Improve `specpilot --help` output — add an `.addHelpText('after', ...)` block in `cli.ts` that prints a tip listing all command aliases (e.g. `init → i`, `validate → v`, etc.) and a note that per-command options are available via `specpilot <command> --help`
