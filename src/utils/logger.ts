...
export class Logger {
  ...
  displayWelcome(): void {
    const content = [
      ...
      chalk.white('  • init|i [options] <name>    - Initialize new SDD project'),
      chalk.white('  • init --dry-run             - Preview files that would be created without writing'),
      chalk.white('  • add-specs|add [options]    - Add specs to existing project'),
      chalk.white('  • validate|v [options]       - Validate specification files'),
      chalk.white('  • archive|ar [options]       - Archive oversized prompts.md / tasks.md entries'),
      chalk.white('  • backfill|bf [options]     - Backfill missing mandates into existing projects'),
      chalk.white('  • list|ls [options]          - Show available templates'),
      chalk.white('  • migrate|m [options]        - Convert legacy .project-spec folder (rarely needed)'),
      chalk.white('  • refine|ref [options] [desc]- Refine project specifications from natural language description'),
      '',
      chalk.cyan('Aliases:'),
      chalk.white('  init→i  validate→v  migrate→m  list→ls  refine→ref  archive→ar  add-specs→add  backfill→bf'),
      '',
