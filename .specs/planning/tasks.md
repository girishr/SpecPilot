...
1. [CS-068] Token optimization — generated `prompts.md` template + dedicated `onboarding.md` (`specFileGenerator.ts`, `init.ts`, `add-specs.ts`)
   ...
   - Edit B: Add `generateOnboardingMd()` in `specFileGenerator.ts` — writes `.specs/development/onboarding.md` with a self-destruct header ("delete after use") and the appropriate prompt (greenfield or brownfield), selected via new `context.projectType: 'greenfield' | 'brownfield'` field on `TemplateContext`; method returns rendered prompt string for stdout printing
   - Edit C: Add `projectType: 'greenfield' | 'brownfield'` to `TemplateContext` (templateEngine.ts) and `SpecGeneratorOptions` (specGenerator.ts); `generateSpecs()` returns `{ onboardingPrompt: string }` instead of `void`
   - Edit D: Add greenfield/brownfield selection prompt in `init.ts` and `add-specs.ts` — list prompt placed as first interactive prompt in both commands; `init` defaults to `greenfield` under `--no-prompts`, `add-specs` defaults to `brownfield`
   - Edit E: After `generateSpecs()` completes in `init.ts` and `add-specs.ts`, print the onboarding prompt to stdout in a highlighted block with a note that it is also saved to `onboarding.md`
   - Edit F: Update `NEW_PROJECT_README` and `EXISTING_PROJECT_README` Quick Start in `specFileGenerator.ts` to point to `onboarding.md` instead of the removed sections in `prompts.md` (absorbs CS-072)
