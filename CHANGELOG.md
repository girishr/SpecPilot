...
- **`specpilot backfill` command** (CD-115/CS-050): new `src/utils/specBackfiller.ts` uses fingerprint-based detection (not yaml.dump) to find mandates missing from `project.yaml` and `copilot-instructions.md`; text-based insertion preserves all comments, emoji, and formatting; if `copilot-instructions.md` doesn't exist it is created in full; `src/commands/backfill.ts` prints a per-file result summary; registered in `cli.ts` as `backfill` / alias `bf` with `--dir`, `--specs-name`, `--dry-run`; welcome screen and `--help` aliases line updated; `migrate` description corrected everywhere from “Migrate between spec versions” to “Convert legacy `.project-spec` folder (rarely needed)”.
