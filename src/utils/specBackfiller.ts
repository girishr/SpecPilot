...
export interface IdeFileBackfillResult {
  path: string;
  action: 'updated' | 'created' | 'skipped' | 'missing' | 'stale' | 'warn';
  found: number;
  total: number;
  added: string[];
  reason?: string;
...
export class SpecBackfiller {
  ...
  private backfillIdeFiles(projectDir: string, dryRun: boolean): IdeFileBackfillResult[] {
    ...
    if (
      existsSync(join(projectDir, '.cursor', 'rules', 'project.mdc')) &&
      !existsSync(join(projectDir, '.cursor', 'rules', 'specpilot.mdc'))
    ) {
      results.unshift({
        ...
        added: [],
        reason:
          'SpecPilot now generates .cursor/rules/specpilot.mdc. If this file was generated by SpecPilot, manually rename it to avoid duplicate rules in Cursor.',
      });
    }
