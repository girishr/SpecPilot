...
1. [CS-070] [BL-036] Make `api.yaml` generation conditional — `generateApiYaml()` currently outputs all three paradigm sections (REST/OpenAPI, CLI, GraphQL) for every project regardless of `context.framework`; route on `context.framework` to emit only the relevant section (e.g. `express`/`fastapi`/`django`/`spring` → REST; CLI-flagged frameworks → CLI block; `graphql` → GraphQL); skip file entirely if no API paradigm is detectable
